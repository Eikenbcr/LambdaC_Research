//Code used to create pull plots
//Code also used to test different fits
//(Same as FitFunctionAnalysis.C but with pull plots)

#define PullDistribution_cxx
#include "PullDistribution.h"
...
void PullDistribution::Begin(TTree * /*tree*/)
{
   ...
   TString option = GetOption();
      ...
      File = new TFile("PullPlots.root", "RECREATE");
     gFile = File;
...
void PullDistribution::SlaveBegin(TTree * /*tree*/)
{
   TString option = GetOption();
}

Bool_t PullDistribution::Process(Long64_t entry)
{
  GetEntry(entry);
...
void PullDistribution::SlaveTerminate()
{
}

void PullDistribution::Terminate()
{
  Double_t sigma;
  Double_t deltaSigma;
  Double_t mu;
